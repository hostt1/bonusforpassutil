...
impl Serialize for Color {
    ...
    where
        S: serde::Serializer,
    {
        let color = self.as_rgba8();

        let bytes = [
            b'#',
            hex_to_ascii_byte((color.0 & 0xf0) >> 4).unwrap(),
            hex_to_ascii_byte(color.0 & 0xf).unwrap(),
            hex_to_ascii_byte((color.1 & 0xf0) >> 4).unwrap(),
            hex_to_ascii_byte(color.1 & 0xf).unwrap(),
            hex_to_ascii_byte((color.2 & 0xf0) >> 4).unwrap(),
            hex_to_ascii_byte(color.2 & 0xf).unwrap(),
            hex_to_ascii_byte((color.3 & 0xf0) >> 4).unwrap(),
            hex_to_ascii_byte(color.3 & 0xf).unwrap(),
        ];
...
impl<'de> Visitor<'de> for ColorVisitor {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("a string with # in front and 6 or 8 hex numbers")
    }
...
const fn hex_to_ascii_byte(b: u8) -> Result<u8, u8> {
    match b {
        0..=9 => Ok(b'0' + b),
        0xA..=0xF => Ok(b'a' + b - 10),
        _ => Err(b),
